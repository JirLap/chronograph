...
class Utils {
	...
	static roundDurationForProperties(time) {
		...
		// always in minutes
		if (_parsedTime > 1e3 * 60) return parseInt(_parsedTime / (1e3 * 60)); //others
		if (_parsedTime > 1e3) return parseFloat(parseFloat(_parsedTime / (1e3 * 60)).toFixed(1)); //secc
		return parseFloat(parseFloat(_parsedTime / (1e3 * 60)).toFixed(2)); //ms
		/*
